...
public class GetDeviceId {
    ...
    //保存文件的路径
    private static final String CACHE_IMAGE_DIR = "devices";
    private static final String ACTIVATION_DIR = "activation";
    //保存的文件 采用隐藏文件的形式进行保存
    private static final String DEVICES_FILE_NAME = ".DEVICES";
    private static final String ACTIVATION_NAME = ".ACTIVATION";

    public static String getActivationCode(Context context) {
        String deviceId = readActivationCode(context);
        return deviceId;
    }
    /**
     * 获取设备唯一标识符
...
//        }
    ...
    public static String readActivationCode(Context context) {
        File file = getActivationDir(context);
        StringBuffer buffer = new StringBuffer();
        try {
            FileInputStream fis = new FileInputStream(file);
            InputStreamReader isr = new InputStreamReader(fis, "UTF-8");
...
//        return info.getMacAddress();
    ...
    public static void saveDeviceID(String str, Context context) {
        File file = getDevicesDir(context);
        try {
            FileOutputStream fos = new FileOutputStream(file);//context.openFileOutput(DEVICES_FILE_NAME, Context.MODE_PRIVATE);//
    ...
    public static void saveActivationcode(String str, Context context) {
        File file = getActivationDir(context);
        try {
            FileOutputStream fos = new FileOutputStream(file);//context.openFileOutput(DEVICES_FILE_NAME, Context.MODE_PRIVATE);//
            Writer out = new OutputStreamWriter(fos, "UTF-8");
    ...
    private static File getDevicesDir(Context context) {
        ...
        if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
            ...
            File cropdir = new File(context.getCacheDir(), CACHE_IMAGE_DIR);
            if (!cropdir.exists()) {
                cropdir.mkdirs();
            }
    ...
    private static File getActivationDir(Context context) {
        File mCropFile = null;
        File cropdir = new File(context.getCacheDir(), ACTIVATION_DIR);
        if (!cropdir.exists()) {
            cropdir.mkdirs();
        }
        mCropFile = new File(cropdir, ACTIVATION_NAME);
        return mCropFile;
    }
}